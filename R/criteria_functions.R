...
eval.criteria <- function(allele.props.array, candidates, subset=5){
  ...
  for(i in 1:nrow(possible)){
    # L2norm
    mean.L2norm[i] <- mean.pairwise.L2norm(allele.props.array=allele.props.array, these.individuals=possible[i,])
    
    this.allele.freq <- get.allele.freq(allele.props.array=allele.props.array, these.individuals=possible[i,])
    # Imbalance
    this.set.imbalance <- flag.allele.imbalance(allele.freq=this.allele.freq)
    number.imbalance[i] <- length(this.set.imbalance)
    # Extreme imbalance
    this.set.extreme.imbalance <- flag.allele.imbalance(allele.freq=this.allele.freq, cut.off.freq=1)
    number.extreme.imbalance[i] <- length(this.set.extreme.imbalance)
    # Wild-derived allele loss
    this.set.wild.loss <- flag.wild.allele.loss(allele.freq=this.allele.freq)
    ...
    this.set.pwk_cast.mean <- mean.pwk_cast.alleles(allele.freq=this.allele.freq)
